...
describe("getDailyGuess / saveDailyGuess", () => {
  ...
  it("round-trips a saved guess for its exact (range, date) pair", () => {
    saveDailyGuess("1M", "2026-08-20", 42.5);

    expect(getDailyGuess("1M", "2026-08-20")).toBe(42.5);
  });

  it("keeps guesses for different dates independent", () => {
    saveDailyGuess("1M", "2026-08-20", 10);
    saveDailyGuess("1M", "2026-08-21", 99);

    expect(getDailyGuess("1M", "2026-08-20")).toBe(10);
    expect(getDailyGuess("1M", "2026-08-21")).toBe(99);
  });
  ...
  it("keeps guesses for the same date on different ranges independent (per issue: 1M/3M/1Y can genuinely differ on the same date)", () => {
    saveDailyGuess("1M", "2026-08-20", 10);
    saveDailyGuess("3M", "2026-08-20", 25);
    saveDailyGuess("1Y", "2026-08-20", 99);

    expect(getDailyGuess("1M", "2026-08-20")).toBe(10);
    expect(getDailyGuess("3M", "2026-08-20")).toBe(25);
    expect(getDailyGuess("1Y", "2026-08-20")).toBe(99);
  });
  ...
  it("overwrites a previous guess for the same (range, date) pair", () => {
    saveDailyGuess("1M", "2026-08-20", 10);
    saveDailyGuess("1M", "2026-08-20", 20);

    expect(getDailyGuess("1M", "2026-08-20")).toBe(20);
  });
  ...
  it("treats a negative stored guess as invalid/absent (no real form submission can produce one, so it can only be hand-edited/corrupt)", () => {
    window.localStorage.setItem("hikt:daily-guess:1M:2026-08-20", JSON.stringify({ guess: -5 }));

    expect(getDailyGuess("1M", "2026-08-20")).toBeNull();
  });

  it("still accepts a stored guess of exactly 0 (a legitimate guess -- 'it went to zero')", () => {
    window.localStorage.setItem("hikt:daily-guess:1M:2026-08-20", JSON.stringify({ guess: 0 }));

    expect(getDailyGuess("1M", "2026-08-20")).toBe(0);
  });
  ...
  it("degrades to a no-op read/write when localStorage itself throws, without crashing the caller", () => {
    ...
    expect(() => saveDailyGuess("1M", "2026-08-20", 5)).not.toThrow();
    expect(getDailyGuess("1M", "2026-08-20")).toBeNull();
  });
});
