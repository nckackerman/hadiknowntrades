...
describe("useCustomResults", () => {
  ...
  it("starts in the loading state when anchor is non-null", () => {
    ...
    const { result } = renderHook(() => useCustomResults("2019-03-15"));
  ...
  it("transitions to error with the API's error/message shape on a non-2xx response", async () => {
    const body = {
      error: "not_found",
      message: 'No precomputed results are available for the custom start date "2019-03-15".',
    };
  ...
  it("requests the anchor-specific endpoint", async () => {
    ...
    renderHook(() => useCustomResults("2019-03-15"));

    await waitFor(() => expect(fetch).toHaveBeenCalledWith("/api/results?anchor=2019-03-15"));
  });
  ...
  it("re-fetches and resets to loading when the anchor changes", async () => {
    ...
    const { result, rerender } = renderHook(
      ...
      { initialProps: { anchor: "2019-03-15" } },
    );
    await waitFor(() => expect(result.current?.status).toBe("success"));

    rerender({ anchor: "2020-01-15" });
    expect(result.current).toEqual({ status: "loading" });

    await waitFor(() => expect(fetch).toHaveBeenLastCalledWith("/api/results?anchor=2020-01-15"));
  });
  ...
  it("resets to null (stops fetching) when the anchor becomes null", async () => {
    ...
    const { result, rerender } = renderHook(
      ({ anchor }: { anchor: string | null }) => useCustomResults(anchor),
      { initialProps: { anchor: "2019-03-15" } as { anchor: string | null } },
    );
    await waitFor(() => expect(result.current?.status).toBe("success"));
