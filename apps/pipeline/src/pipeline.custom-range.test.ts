...
describe("runPipeline custom-range anchors (issue #11, day-granularity since issue #75)", () => {
  it("computes and writes a CustomWindowResult for every real trading day within the lookback window, plus a manifest", async () => {
    ...
    // The Cut's own 7 per-range results (issue #232, extended to
    // CUT_RANGES' 7th "1D" entry by issue #238, reusing this same
    // fixture's real AAPL data -- see results/sp500-prefix/*.json below).
    expect(summary.results).toHaveLength(6);
    expect(store.objects.size).toBe(6 + 3 + 1 + 7);
  ...
  it("defaults to zero custom anchors, and writes no manifest, when computeCustomAnchors is omitted", async () => {
    ...
    // extended to CUT_RANGES' 7th "1D" entry by issue #238) --
  ...
  it("a custom-anchor write failure aggregates into the same thrown error as a preset failure, without blocking sibling writes", async () => {
    ...
    // Cut's own 7 per-range results (issue #232, extended to CUT_RANGES'
    // 7th "1D" entry by issue #238) all still landed despite the one
    // custom-anchor write failure -- "write whatever succeeded" is
    // preserved across every family, not just within the preset-range
    // set.
    expect(objects.size).toBe(6 + 1 + 1 + 7);
